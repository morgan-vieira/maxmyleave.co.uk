...
export async function maximiseDaysOff(
    ...
    const normalizedRequiredDates = Array.from(new Set((requiredLeaveDates ?? [])
        .map((date) => dayjs(date))
    ...
    const remainingAllowance = Math.max(leaveAllowance - mandatoryWorkdayCount, 0);
...
function formatResult(
    ...
    for (let idx = opportunity.leaveStartIdx; idx <= opportunity.leaveEndIdx; idx++) {
        if (calendar[idx].isWorkday) {
            leaveDates.push(calendar[idx].date.format("YYYY-MM-DD"));
        }
    }

    const breakStartDate = calendar[opportunity.breakStartIdx].date.format("YYYY-MM-DD");
    const breakEndDate = calendar[opportunity.breakEndIdx].date.format("YYYY-MM-DD");
    const leaveStartDate = leaveDates[0] ?? breakStartDate;
    const leaveEndDate = leaveDates[leaveDates.length - 1] ?? breakEndDate;

    const overlappingMandatoryDates = leaveDates.filter((date) => mandatoryDateSet.has(date));
    const totalLeaveDays = leaveDates.length;
    const additionalLeaveNeeded = Math.max(0, totalLeaveDays - overlappingMandatoryDates.length);
    const breakLength = opportunity.breakLength;
    const freeDays = Math.max(0, breakLength - totalLeaveDays);
    const efficiency = additionalLeaveNeeded > 0
...
function generateClusterExtensions(
    ...
    for (let before = 0; before <= Math.min(maxBefore, leaveAllowance); before++) {
        for (let after = 0; after <= Math.min(maxAfter, leaveAllowance - before); after++) {
            if (before === 0 && after === 0) {
                continue;
            }
            ...
            const type: LeaveOpportunity['type'] = before > 0 && after > 0 ? 'bridge' : 'extension';
...
function generateClusterBridges(
    ...
    for (let i = 0; i < clusters.length - 1; i++) {
        ...
        if (gapWorkdays <= 0 || gapWorkdays > leaveAllowance || gapWorkdays > MAX_BRIDGE_GAP) {
            continue;
        }
        ...
        for (let before = 0; before <= Math.min(maxBefore, leaveAllowance - gapWorkdays); before++) {
            for (let after = 0; after <= Math.min(maxAfter, leaveAllowance - gapWorkdays - before); after++) {
...
function findStandaloneOpportunities(
    ...
    for (let i = 0; i <= calendar.length - 4; i++) {
        ...
        if (saturday.weekday !== 6 || sunday.weekday !== 0 || monday.weekday !== 1) {
            continue;
        }
        ...
        const fridayAdditional = Math.max(0, fridayStats.totalLeaveDays - fridayStats.overlapCount);

        if (fridayStats.totalLeaveDays > 0 && fridayAdditional <= remainingAllowance) {
            const fridayEfficiency = fridayAdditional > 0 ? fridayTotal / fridayAdditional : fridayTotal;
            const fridayDescription = fridayAdditional === 0
        ...
        const mondayStats = evaluateLeaveWindow(calendar, i + 3, i + 3, mandatoryIndices);
        const mondayAdditional = Math.max(0, mondayStats.totalLeaveDays - mondayStats.overlapCount);

        if (mondayStats.totalLeaveDays > 0 && mondayAdditional <= remainingAllowance) {
            const mondayEfficiency = mondayAdditional > 0 ? mondayTotal / mondayAdditional : mondayTotal;
            const mondayDescription = mondayAdditional === 0
        ...
        if (maxDays >= 2) {
            ...
            const combinedTotal = combinedRange.breakEndIdx - combinedRange.breakStartIdx + 1;
            const combinedStats = evaluateLeaveWindow(calendar, i, i + 3, mandatoryIndices);
            const combinedAdditional = Math.max(0, combinedStats.totalLeaveDays - combinedStats.overlapCount);

            if (combinedStats.totalLeaveDays > 0 && combinedAdditional <= remainingAllowance) {
                const combinedEfficiency = combinedAdditional > 0 ? combinedTotal / combinedAdditional : combinedTotal;
                const combinedDescription = combinedAdditional === 0
...
function selectNonOverlappingOpportunities(
    ...
    const groups = Array.from(groupMap.entries()).map(([anchorKey, variants]) => {
        const variantList = Array.from(variants.values()).sort((a, b) => a.leaveDaysUsed - b.leaveDaysUsed);
    ...
    const primaryVariants = groups.map((group) => ({
        anchorKey: group.anchorKey,
        opportunity: group.variants[0]
    })).filter((entry) => entry.opportunity !== undefined) as Array<{ anchorKey: string; opportunity: LeaveOpportunity }>;

    const secondaryVariants: Array<{ anchorKey: string; opportunity: LeaveOpportunity }> = [];
    ...
    for (let i = 0; i < primaryVariants.length && selected.length < primaryQuota; i++) {
...
function isOpportunityBetter(candidate: LeaveOpportunity, baseline: LeaveOpportunity): boolean {
    ...
    const candidateCoverage = Number(candidate.hasLeadingLeave) + Number(candidate.hasTrailingLeave);
    const baselineCoverage = Number(baseline.hasLeadingLeave) + Number(baseline.hasTrailingLeave);
...
async function getHolidayNames(region: string = "england-and-wales"): Promise<Array<{ title: string; date: string }>> {
    try {
...
function expandBreakBounds(
    ...
    while (breakEndIdx < calendar.length - 1 && !calendar[breakEndIdx + 1].isWorkday) {
        breakEndIdx++;
    }
