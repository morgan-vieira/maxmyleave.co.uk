...
export async function calculateLeaveAction(formData: FormData): Promise<PlannerResult> {
    try {
        // Extract form data
        ...
        const requiredLeaveDates = mustHaveDates.filter((value): value is string => typeof value === 'string')

        // Calculate optimal leave dates
        const plan = await maximiseDaysOff(leaveDays, planningYear, region, 5, requiredLeaveDates)
