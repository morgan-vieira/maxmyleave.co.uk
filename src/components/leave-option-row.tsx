...
export default function LeaveOptionRow({ result, index, variants = [], availableAllowance, totalAllowance, mandatoryLeaveCount = 0 }: LeaveOptionRowProps) {
    ...
    const baseAllowance = totalAllowance ?? (availableAllowance !== undefined ? availableAllowance + mandatoryLeaveCount : undefined);
    const projectedAllowance = baseAllowance !== undefined
        ? Math.max(baseAllowance - (mandatoryLeaveCount + result.leaveDaysUsed), 0)
        : undefined;
    ...
    const summaryLines: Array<{ label: string; value: string; helper?: string }> = [
        {
            label: 'Book',
            value: hasZeroAdditional ? 'No new days' : `${formatCount(result.leaveDaysUsed, 'day')} of annual leave`,
            helper: hasZeroAdditional ? 'All covered by your pinned days' : 'Request this time off to make it happen'
        },
        {
            label: 'Time off',
    ...
    return (
        ...
        >
            <div className="text-xs uppercase tracking-wide text-muted-foreground">Break {index + 1}</div>
            <div className="text-base font-semibold text-foreground transition-colors duration-[400ms]">{breakRangeLabel}</div>

            <div className="mt-2 text-sm text-muted-foreground transition-colors duration-[400ms]">
                {result.description}
            </div>

            <dl className="mt-4 grid gap-2 text-sm text-muted-foreground transition-colors duration-[400ms] sm:grid-cols-2">
                {summaryLines.map((line) => (
                    ...
                    >
                        ...
                        <dd className="text-base font-semibold text-foreground transition-colors duration-[400ms]">{line.value}</dd>
                        {line.helper && <span className="text-xs text-muted-foreground">{line.helper}</span>}
                    </div>
                ))}
            </dl>
            ...
            {result.overlappingMandatoryDates.length > 0 && (
                ...
                >
                    📌 Covers your must-have days: {result.overlappingMandatoryDates.map(date => new Date(date).toLocaleDateString('en-GB', { day: '2-digit', month: 'short' })).join(', ')}
                </div>
            )}
            ...
            {sortedVariants.length > 0 && (
                ...
                >
                    ...
                    {sortedVariants.map((variant, variantIndex) => {
                        ...
                        const variantAdditional = variant.leaveDates.filter((date) => !variantOverlap.has(date));
                        const variantSummary = variantAdditional.length > 0
                        ...
                        return (
                            <div key={`${variant.breakStartDate}-${variant.breakEndDate}-${variantIndex}`}>
                                Book {formatCount(variant.leaveDaysUsed, 'day')} → enjoy {formatCount(variant.breakLength, 'day')}. {variantSummary}.
                            </div>
                        );
                    })}
                </div>
            ...
            >
                With {formatCount(result.leaveDaysUsed, 'day')} booked, you can take {formatCount(result.breakLength, 'day')} off.
            </div>
        </div>
    );
}