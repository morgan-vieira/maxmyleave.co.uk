...
function TooltipCell({ className, tooltipLines, children, style }: TooltipCellProps) {
    ...
    return (
        <div className="group relative w-full">
            ...
            {tooltipLines.length > 0 && (
                <div
                    className="pointer-events-none absolute left-1/2 bottom-full z-20 hidden w-max max-w-[14rem] -translate-x-1/2 -translate-y-2 rounded-md border px-2 py-1 text-[10px] leading-snug shadow-lg group-hover:flex group-focus-within:flex"
                    style={{
...
export default function LeaveOptionsDisplay({
    ...
    const formatDisplayDate = (date: Date): string => date.toLocaleDateString('en-GB', {
        day: '2-digit',
        month: 'short',
        year: 'numeric'
    });
    ...
    const normalisedResults = useMemo(() => {
        ...
        for (const result of results) {
            ...
            const shouldReplace =
                ...
                (result.leaveDaysUsed === existing.leaveDaysUsed && result.efficiency > existing.efficiency) ||
                (result.leaveDaysUsed === existing.leaveDaysUsed && result.efficiency === existing.efficiency && new Date(result.breakStartDate).getTime() < new Date(existing.breakStartDate).getTime());
        ...
        uniqueResults.sort((a, b) => {
            const startDiff = new Date(a.breakStartDate).getTime() - new Date(b.breakStartDate).getTime();
            if (startDiff !== 0) {
                return startDiff;
            }

            const endDiff = new Date(a.breakEndDate).getTime() - new Date(b.breakEndDate).getTime();
            if (endDiff !== 0) {
                return endDiff;
            }
    ...
    const groupedBreaks = useMemo(() => {
        ...
        const grouped = Array.from(groupMap.values()).map((group) => {
            const sortedGroup = group.slice().sort((a, b) => {
                ...
                return new Date(a.breakEndDate).getTime() - new Date(b.breakEndDate).getTime();
            });
        ...
        grouped.sort((a, b) => {
            const startDiff = new Date(a.primary.breakStartDate).getTime() - new Date(b.primary.breakStartDate).getTime();
            if (startDiff !== 0) {
                return startDiff;
            }
            return new Date(a.primary.breakEndDate).getTime() - new Date(b.primary.breakEndDate).getTime();
        });
    ...
    const monthGroups = useMemo(() => {
        if (groupedBreaks.length === 0) {
            return [] as Array<{
                ...
                year: number;
                month: number;
                entries: Array<{ primary: LeaveResult; variants: LeaveResult[]; displayIndex: number }>;
            }>;
        }
    ...
    const aggregateSummary = useMemo(() => {
        ...
        const totals = normalisedResults.reduce<{ totalLeaveDays: number; totalBreakDays: number }>((acc, option) => {
        ...
        const remainingAfterAll = allowanceBaseline !== null
    ...
    const summaryCards = useMemo(() => {
        ...
        if (normalisedResults.length > 0) {
            const longestBreak = normalisedResults.reduce((max, option) => Math.max(max, option.breakLength), 0);
            if (longestBreak > 0) {
                cards.push({
        ...
        return cards;
    }, [annualLeaveDays, remainingAllowance, normalisedResults, mandatoryLeaveDates.length]);
    ...
    const pinnedCalendarMonths = useMemo(() => {
        const monthMap = new Map<string, { year: number; month: number; mandatory: Set<string>; nonWorking: Set<string> }>();
    ...
    const buildPinnedCalendar = (
        ...
        const days: Array<{ date: Date; isCurrentMonth: boolean; status: 'mandatory' | 'nonWorking' | null }> = [];
    ...
    return (
        <div className="space-y-6">
            {summaryCards.length > 0 && (
                <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                    {summaryCards.map((card) => (
                        ...
                        >
                            <div className="text-xs uppercase tracking-wide text-muted-foreground transition-colors duration-[400ms]">
                                {card.label}
                            </div>
                            <div className="mt-2 text-3xl font-semibold text-foreground transition-colors duration-[400ms]">{card.value}</div>
                            {card.helper && (
                                <div className="mt-1 text-xs text-muted-foreground transition-colors duration-[400ms]">{card.helper}</div>
                            )}
                        </div>
                    ))}
                </div>
            ...
            {!hasBreakResults && (
                <div
                    className="border rounded-lg p-6 shadow-sm text-sm text-muted-foreground transition-colors duration-[400ms]"
                    style={{
                        backgroundColor: 'rgb(var(--surface) / 0.9)',
                        borderColor: 'rgb(var(--border))',
                        boxShadow: '0 12px 32px rgb(var(--foreground) / 0.05)'
                ...
                >
                    No leave combinations fit within your remaining allowance right now. Adjust your must-have dates or allowance to explore more options.
                </div>
            )}

            {monthGroups.map((group) => {
                const breakSummaries = group.entries.map(({ primary, variants, displayIndex }) => {
                ...
                for (let i = 0; i < 42; i++) {
                    ...
                    const activeBreaks = breakSummaries.filter((summary) => cellDate >= summary.start && cellDate <= summary.end);
                    const isLeaveDay = activeBreaks.some((summary) => summary.leaveSet.has(dateKey));
                    const tooltipParts: string[] = [cellDate.toLocaleDateString('en-GB', { weekday: 'short', day: 'numeric', month: 'short' })];
                    let classes = "relative flex items-center justify-center rounded border p-2 text-sm transition-colors duration-200 ";
                    ...
                    if (activeBreaks.length > 0) {
                        ...
                        tooltipParts.push(...activeBreaks.map((summary) => `${summary.label}: ${describeDay(summary)}`));
                    } else if (isHoliday) {
                ...
                return (
                    <div key={group.key} className="space-y-4">
                        <div className="text-lg font-semibold text-foreground transition-colors duration-[400ms]">
                            {group.label}
                        </div>
                        <div className="space-y-4">
                            <div>
                                <h5 className="text-sm font-medium text-foreground transition-colors duration-[400ms] mb-3">
                                ...
                                <div className="space-y-4">
                                    {group.entries.map(({ primary, variants, displayIndex }) => (
                                        <LeaveOptionRow
                                            ...
                                            result={primary}
                                            index={displayIndex}
                                            availableAllowance={typeof remainingAllowance === 'number' ? remainingAllowance : undefined}
                            ...
                            >
                                ...
                                <div className="mt-1 grid grid-cols-7 gap-1">
                                    {calendarCells.map((cell, cellIndex) => (
                                        <TooltipCell key={`${cell.date.toISOString()}-${cellIndex}`} className={cell.classes} tooltipLines={cell.tooltipLines} style={cell.style}>
                                            {cell.date.getDate()}
                                        </TooltipCell>
                                    ))}
                                </div>
                                <div className="mt-3 flex flex-wrap justify-center gap-4 text-[11px] text-muted-foreground">
                                    ...
                                    <div className="flex items-center gap-1">
                                        <span
                                            className="inline-block h-3 w-3 rounded"
                                            style={{ backgroundColor: 'rgb(var(--calendar-holiday) / 0.65)' }}
                                        ></span>
                                    ...
                                    <div className="flex items-center gap-1">
                                        <span
                                            className="inline-block h-3 w-3 rounded"
                                            style={{ backgroundColor: 'rgb(var(--calendar-weekend) / 0.65)' }}
                                        ></span>
                                        <span>Weekend</span>
                                    </div>
                                </div>
            ...
            {hasPinnedMonths && (
                <div className="space-y-4">
                    ...
                    <div className="text-sm text-muted-foreground space-y-1">
                        {mustHaveRange && (
                            <div>
                                These must-stay-free dates span {mustHaveRange.start.toLocaleDateString('en-GB', { day: '2-digit', month: 'short' })} → {mustHaveRange.end.toLocaleDateString('en-GB', { day: '2-digit', month: 'short' })}.
                            </div>
                        )}
                        {hasIgnored && (
                            <div>
                                We skipped {ignoredMandatoryDates.map(date => new Date(date).toLocaleDateString('en-GB', { day: '2-digit', month: 'short' })).join(', ')} because they fall outside this planner's year.
                            </div>
                        )}
                        {overbooked && (
                            <div style={{ color: 'rgb(var(--destructive))' }}>
                                ⚠️ You've selected more must-have leave than your allowance covers. Consider freeing up days or increasing your allowance.
                            </div>
                        )}
                    </div>

                    <div className="grid gap-4 sm:grid-cols-2">
                        {pinnedCalendarMonths.map(({ year: calendarYear, month: calendarMonth, mandatory, nonWorking }) => {
                            const monthLabel = new Date(calendarYear, calendarMonth, 1).toLocaleDateString('en-GB', {
                                month: 'long',
                                year: 'numeric'
                            });
                            const days = buildPinnedCalendar(calendarYear, calendarMonth, mandatory, nonWorking);
                            const holidaySet = new Set(holidays);
                            ...
                            return (
                                ...
                                >
                                    <h5 className="text-sm font-medium text-foreground transition-colors duration-[400ms] mb-3 text-center">
                                        {monthLabel}
                                    </h5>
                                    <div className="grid grid-cols-7 gap-1 text-[11px] text-muted-foreground">
                                        {DAY_LABELS.map((day) => (
                                    ...
                                    <div className="mt-1 grid grid-cols-7 gap-1">
                                        {days.map(({ date, isCurrentMonth, status }, index) => {
                                            ...
                                            const isHoliday = holidaySet.has(dateKey);
                                            let classes = "text-center p-2 text-sm rounded border transition-colors duration-200 ";
                                            const tooltipParts: string[] = [date.toLocaleDateString('en-GB', { weekday: 'short', day: 'numeric', month: 'short' })];
                                            const style: CSSProperties = {
                                                borderColor: 'rgb(var(--border) / 0.5)',
                                                backgroundColor: 'rgb(var(--surface-muted) / 0.45)',
                                                color: 'rgb(var(--foreground))'
                                            ...
                                            return (
                                                <TooltipCell key={index} className={classes} tooltipLines={tooltipParts} style={style}>
                                                    {date.getDate()}
                                                </TooltipCell>
                                            );
                                        })}
                                    ...
                                    <div className="mt-3 flex flex-wrap justify-center gap-4 text-[11px] text-muted-foreground">
                                        ...
                                        {nonWorking.size > 0 && (
                                            <div className="flex items-center gap-1">
                                                <span
                                                    className="inline-block h-3 w-3 rounded"
                                                    style={{ backgroundColor: 'rgb(var(--calendar-nonworking) / 0.65)' }}
                                                ></span>
            ...
            {aggregateSummary && (
                ...
                >
                    <h5 className="text-sm font-semibold uppercase tracking-wide text-foreground transition-colors duration-[400ms]">
                        planner highlights
                    </h5>
                    <p className="mt-1 text-sm text-muted-foreground transition-colors duration-[400ms]">
                        Totals below reflect every break surfaced by the planner. Pick the combos that fit your allowance and appetite.
                    </p>
                    <div className="mt-4 grid gap-3 sm:grid-cols-3">
                        ...
                        >
                            <div className="text-xs uppercase tracking-wide text-muted-foreground">Proposed leave to book</div>
                            <div className="mt-1 text-xl font-semibold text-foreground transition-colors duration-[400ms]">{formatCount(aggregateSummary.totalLeaveDays, 'day')}</div>
                            <div className="text-xs text-muted-foreground mt-1">Total across all highlighted breaks</div>
                        </div>
                        <div
                            className="rounded-md border p-3 transition-colors duration-[400ms]"
                            style={{
                        ...
                        >
                            <div className="text-xs uppercase tracking-wide text-muted-foreground">Time off unlocked</div>
                            <div className="mt-1 text-xl font-semibold text-foreground transition-colors duration-[400ms]">{formatCount(aggregateSummary.totalBreakDays, 'day')}</div>
                            <div className="text-xs text-muted-foreground mt-1">Weekends and bank holidays included</div>
                        </div>
                        {aggregateSummary.allowanceBaseline !== null && (
                            ...
                            >
                                <div className="text-xs uppercase tracking-wide text-muted-foreground">Allowance remaining</div>
                                <div className="mt-1 text-xl font-semibold text-foreground transition-colors duration-[400ms]">{formatCount(aggregateSummary.remainingAfterAll ?? 0, 'day')}</div>
                                <div className="text-xs text-muted-foreground mt-1">If you booked every option shown</div>
                            </div>
                        )}
                    </div>
                </div>
